...
TEST_F(ConcatenationTest, INT64_P)
{
  ...
  std::vector<int64_t> output_data_vector = onert_micro::execute::testing::checkTISOKernel<int64_t>(&test_data_kernel);
...
TEST_F(ConcatenationTest, INT32_P)
{
  ...
  std::vector<int32_t> output_data_vector = onert_micro::execute::testing::checkTISOKernel<int32_t>(&test_data_kernel);
  EXPECT_THAT(output_data_vector, test_data_kernel.get_output_data_by_index(0));
}
...
TEST_F(ConcatenationTest, Float_P)
{
  ...
  std::vector<float> output_data_vector = onert_micro::execute::testing::checkTISOKernel<float>(&test_data_kernel);

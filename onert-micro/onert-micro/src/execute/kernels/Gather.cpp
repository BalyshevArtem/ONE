...
void gather(const InputT *input_data, const CoordsT *coords_data,
            InputT *output_data, int32_t axis_size, int32_t batch_size, int32_t outer_size, int32_t inner_size, int32_t coord_size)
{
...
OMStatus onert_micro::execute::execute_kernel_CircleGather(const OMExecuteArgs &execute_args)
{
  ...
  switch (input->type())
  {
    ...
    case circle::TensorType_FLOAT32: {
      gather<float, int32_t>(utils::castInputData<float>(input_data), utils::castInputData<int32_t>(position_data),
        utils::castOutputData<float>(output_data), axis_size, batch_size, outer_size, inner_size, coord_size);
    }
      break;
#endif // DIS_FLOAT
#ifndef DIS_QUANT
    case circle::TensorType_INT8: {
      gather<int8_t, int32_t>(utils::castInputData<int8_t>(input_data), utils::castInputData<int32_t>(position_data),
                             utils::castOutputData<int8_t>(output_data), axis_size, batch_size, outer_size, inner_size, coord_size);
    }
      break;
#endif // DIS_QUANT
    case circle::TensorType_INT32: {
      gather<int32_t, int32_t>(utils::castInputData<int32_t>(input_data), utils::castInputData<int32_t>(position_data),
                              utils::castOutputData<int32_t>(output_data), axis_size, batch_size, outer_size, inner_size, coord_size);
    }
      break;
    default: {

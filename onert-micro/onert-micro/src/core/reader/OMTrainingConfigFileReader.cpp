...
OMStatus OMTrainingConfigReader::validate(OMCircleReader *reader)
{
  ...
  assert(mag_num == train_config_file_magic_number && "False MAGIC NUMBER, check correctness of wof file");
  ...
  assert(version == train_config_file_schema_version && "False MAGIC NUMBER, check correctness of wof file");
 ...
 */
std::unordered_set<uint16_t> OMTrainingConfigReader::getTrainableOpsIndexes()
{
  ...
  char *cur_ptr = &_train_config_ptr[FIRST_LAYER_INDEX_FIELD];
  // Fill result set with indexes
  for (uint32_t i = 0; i < num_ops_in_file; ++i)
  {
    uint16_t cur_op_index;
    std::memcpy(&cur_op_index, cur_ptr, sizeof(cur_op_index));
    cur_ptr += sizeof(cur_op_index);
    result.insert(cur_op_index);
  }

...
OMStatus onert_micro::import::configure_kernel_CircleGather(const OMConfigureArgs &config_args)
{
  ...
  status = utils::checkCondition(input_type == circle::TensorType_INT32 or input_type == circle::TensorType_FLOAT32
                                 or input_type == circle::TensorType_INT8);
  if (status != Ok)
    return status;
  ...
  status = utils::checkCondition(batch_dims <= axis and batch_dims >= 0 and batch_dims < num_dims and batch_dims <= coords_num_dims);

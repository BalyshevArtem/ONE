...
OMStatus validateWeightTensorSize(const core::OMRuntimeShape &weight_shape, int dim1_size, int dim2_size)
{
  if (weight_shape.dimensionsCount() != 2 or weight_shape.dims(0) != dim1_size or weight_shape.dims(1) != dim2_size)
...
OMStatus validateTensorsSize(execute::lstm::LSTMStruct *lstm_struct, const bool time_major)
{
  ...
  const auto batch_size =
    time_major ? input_shape.dims(1) : input_shape.dims(0);
  ...
  {
    status = validateWeightTensorSize(lstm_struct->get_internal_tensor(i), state_dimension, input_dimension);
    if (status != Ok)
      return status;
  }
  ...
  {
    status = validateWeightTensorSize(lstm_struct->get_internal_tensor(i), state_dimension, state_dimension);
    if (status != Ok)
      return status;
  }
  ...
  if (output_shape.dimensionsCount() != 3 or input_shape.dims(0) != output_shape.dims(0)
      or input_shape.dims(1) != output_shape.dims(1) or output_shape.dims(2) != state_dimension)
    return FailedCheckCondition;
                                                         ...
                                                         core::OMKernel &kernel, const OMConfig&)
{
 OMStatus status = Ok;
  ...
  if (lstm_struct.input()->type() != circle::TensorType_FLOAT32 and lstm_struct.input()->type() != circle::TensorType_INT8)

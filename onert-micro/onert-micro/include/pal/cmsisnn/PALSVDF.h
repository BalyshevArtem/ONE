...
OMStatus SVDF(const core::SVDFQuantParams &params, const int8_t *input_data,
              ...
              const int32_t *bias_data, int8_t *state_data,
              int8_t *output_data, const core::OMRuntimeShape &input_shape,
              ...
              const core::OMRuntimeShape &bias_shape, const core::OMRuntimeShape &output_shape)
{
  ...
  if (buf_size > 0)
  {
    core::memory::OMMemoryManager::allocateMemory(buf_size, &kernel_sums);

    arm_vector_sum_s8(reinterpret_cast<int32_t *>(kernel_sums), input_size, num_filters,
                      weights_feature_data, 1, nullptr);
  }

  cmsis_nn_context ctx;
  ctx.buf = kernel_sums;

  arm_svdf_s8(&ctx, &scratch_ctx, &scratch_output_ctx, &svdf_params, &in_quant_params,
              &out_quant_params, &input_dims, input_data, &state_dims, state_data,
              &weights_feature_dims, weights_feature_data, &weights_time_dims, weights_time_data,
  ...
  core::memory::OMMemoryManager::deallocateMemory(kernel_sums);

...
using OptimizePassFunc = OMGraphStatus (core::OMRuntimeStorage &storage, core::OMRuntimeContext &context, const OMConfig &configs);

#define REGISTER_PASS(name)                        \
  OMGraphStatus onert_micro_##name(core::OMRuntimeStorage &storage, core::OMRuntimeContext &context, const OMConfig &configs);
#include "BuildPass.lst"
#undef REGISTER_PASS
...
public:
  ...
  {
#define REGISTER_PASS(name)                            \
  registerOptimizePasses(optimize::OMPassBuilderID::PASS_##name, \
                            onert_micro_##name);
...
#undef REGISTER_PASS
  ...
  {
    return size_t(optimize::OMPassBuilderID::Size);
  }

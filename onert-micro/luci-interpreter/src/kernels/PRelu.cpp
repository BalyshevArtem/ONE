...
void configure_kernel_CirclePRelu(const circle::Operator *cur_op,
...
void execute_kernel_CirclePRelu(const circle::Operator *cur_op, BaseRuntimeGraph *runtime_graph)
{
  ...
  switch (Tensor::element_type(kernel.input1()))
  {
    ...
    {
      const float *input_data1_float = kernels::getTensorData<float>(tiso_data.input1_data);
      const float *input_data2_float = kernels::getTensorData<float>(tiso_data.input2_data);
      float *output_data_float = kernels::getTensorData<float>(tiso_data.output_data);

      luci_interpreter_pal::BroadcastPRelu4DSlowFloat(kernels::getTensorRuntimeShape(kernel.input1(), runtime_graph),
                                                      input_data1_float,

...
void evalQuantized(const circle::Tensor *input1, const circle::Tensor *input2,
                   ...
                   const circle::AddOptions *options, BaseRuntimeGraph *runtime_graph, DataType type)
{
  ...
  assert(need_broadcast == false && "Broadcast for INT8 not supported now");
...
void execute_kernel_CircleAdd(const circle::Operator *cur_op, BaseRuntimeGraph *runtime_graph)
{
  ...
  switch (type)
  {
    ...
    case DataType::S16:
    {
      evalQuantized(kernel.input1(), kernel.input2(), kernel.output(), options, runtime_graph, type);
    }
    break;
    default:

...
struct LSTMStruct
{
  ...
  void validateTensorTypes()
  {
    ...
    {
      LUCI_INTERPRETER_CHECK(internal_tensors[i] == nullptr or Tensor::element_type(input_to_forget_weights()) == Tensor::element_type(internal_tensors[i]));
    }

    for (int32_t i = 12; i < 16; ++i)
    {
      LUCI_INTERPRETER_CHECK(internal_tensors[i] == nullptr or Tensor::element_type(forget_gate_bias()) == Tensor::element_type(internal_tensors[i]));
    }
  }

  const circle::Tensor *input() { internal_tensors[0]; };

  const circle::Tensor *input_to_input_weights() { internal_tensors[1]; };
  const circle::Tensor *input_to_forget_weights() { internal_tensors[2]; };
  const circle::Tensor *input_to_cell_weights() { internal_tensors[3]; };
  const circle::Tensor *input_to_output_weights() { internal_tensors[4]; };

  const circle::Tensor *recurrent_to_input_weights() { internal_tensors[5]; };
  const circle::Tensor *recurrent_to_forget_weights() { internal_tensors[6]; };
  const circle::Tensor *recurrent_to_cell_weights() { internal_tensors[7]; };
  const circle::Tensor *recurrent_to_output_weights() { internal_tensors[8]; };

  const circle::Tensor *cell_to_input_weights() { internal_tensors[9]; };
  const circle::Tensor *cell_to_forget_weights() { internal_tensors[10]; };
  const circle::Tensor *cell_to_output_weights() { internal_tensors[11]; };

  const circle::Tensor *input_gate_bias() { internal_tensors[12]; };
  const circle::Tensor *forget_gate_bias() { internal_tensors[13]; };
  const circle::Tensor *cell_gate_bias() { internal_tensors[14]; };
  const circle::Tensor *output_gate_bias() { internal_tensors[15]; };

  const circle::Tensor *projection_weights() { internal_tensors[16]; };
  const circle::Tensor *projection_bias() { internal_tensors[17]; };

  const circle::Tensor *output_state() { internal_tensors[18]; };
  const circle::Tensor *cell_state() { internal_tensors[19]; };

  const circle::Tensor *input_layer_norm_coefficients() { internal_tensors[20]; };
  const circle::Tensor *forget_layer_norm_coefficients() { internal_tensors[21]; };
  const circle::Tensor *cell_layer_norm_coefficients() { internal_tensors[22]; };
  const circle::Tensor *output_layer_norm_coefficients() { internal_tensors[23]; };
...
#ifndef DIS_QUANT
struct FullyConnectedQuantizedParams
{
...
{
  FullyConnectedQuantizedParams input_fc_params;
  FullyConnectedQuantizedParams recurrent_fc_params;
};

...
void calculateActivationRangeQuantized(Activation activation, int32_t output_zero_point, float output_scale, DataType data_type,
                                       int32_t *activation_min, int32_t *activation_max)
{
  int32_t qmin{};
  int32_t qmax{};
  ...
  calculateActivationRangeQuantizedImpl(activation, qmin, qmax, output_zero_point, output_scale, activation_min,
                                        activation_max);
}
                                       ...
                                       int32_t *activation_min, int32_t *activation_max)
{
  ...
  const float scale = Tensor::scale(output);
  const int32_t zero_point = Tensor::zero_point(output);
  calculateActivationRangeQuantized(activation, zero_point, scale, Tensor::element_type(output), activation_min, activation_max);
}

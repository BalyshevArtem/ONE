...
int entry(int argc, char **argv)
{
  ...
  // Obtain train config inform
  std::vector<char> train_config_file_data;
  training_configure_tool::createResultData(train_config_file_data);
  ...
  const uint32_t num_train_layers = 11;
  ...
  config.train_mode = true;
  {
    ...
    // Save train config ptr
    train_context.training_config_info_data = train_config_file_data.data();

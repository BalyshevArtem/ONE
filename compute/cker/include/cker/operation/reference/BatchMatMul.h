...
inline void BatchMatMul(const Shape &lhs_shape, const uint8_t *lhs_data, const Shape &rhs_shape,
                       const uint8_t *rhs_data, const Shape &, uint8_t *output_data)
{
  const Shape extended_lhs_shape = Shape::ExtendedShape(5, lhs_shape);
  const Shape extended_rhs_shape = Shape::ExtendedShape(5, rhs_shape);
  ...
  for (int b0 = 0; b0 < batch_dim0; ++b0)
  {
    const uint8_t *lhs_ptr0 = lhs_data + (b0 * lhs_ext0);
    const uint8_t *rhs_ptr0 = rhs_data + (b0 * rhs_ext0);
    for (int b1 = 0; b1 < batch_dim1; ++b1)
    {
      const uint8_t *lhs_ptr1 = lhs_ptr0 + b1 * lhs_ext1;
      const uint8_t *rhs_ptr1 = rhs_ptr0 + b1 * rhs_ext1;
      for (int b2 = 0; b2 < batch_dim2; ++b2)
      {
        const uint8_t *lhs_ptr2 = lhs_ptr1 + b2 * lhs_ext2;
        const uint8_t *rhs_ptr2 = rhs_ptr1 + b2 * rhs_ext2;
        uint8_t *out_ptr = output_data + ((b0 * batch_dim1 * batch_dim2) + b1 * batch_dim2 + b2) *
                                       lhs_rows * rhs_cols;
        for (int j = 0; j < rhs_cols; ++j)
        {
          for (int i = 0; i < lhs_rows; ++i)

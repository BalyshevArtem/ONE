...
void connect(luci::ConnectNode *cn, const luci::CircleSVDF *node)
{
  ...
  auto *weight_feature = loco::must_cast<luci::CircleNode *>(node->weight_feature());
  auto *weight_time = loco::must_cast<luci::CircleNode *>(node->weight_time());
  auto *bias = loco::must_cast<luci::CircleNode *>(node->bias());
  auto *input_activation_state =

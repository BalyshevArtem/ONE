...
{
  uint8_t* memPool = new uint8_t[3 * 1048576];
  ...
  uint8_t *array_1 = (uint8_t *)first_tensor.data<void>();
  for (int i = 0; i < 5; i++)
  {
    EXPECT_EQ(data_1[i], array_1[i]);
  }

  Tensor second_tensor(DataType::U8, Shape({5}), AffineQuantization{}, "second_tensor");
  buddy_memory_manager->allocate_memory(&second_tensor);

  uint8_t data_2 [] = {11, 22, 33, 44, 55};
  second_tensor.writeData(data_2, 5);

  uint8_t *array_2 = (uint8_t *)second_tensor.data<void>();
  for (int i = 0; i < 5; i++)
  {
    EXPECT_EQ(data_2[i], array_2[i]);
  }
